 ...
 <div class="container-fluid">
 	<div class="row" style="margin-bottom: 10px">
 		<div class="col-md-4">
 			<!-- <button type="button" class="btn btn-success" data-toggle="modal" data-target="#modalKonfirmasiexport">Export</button> -->
 			...
 			<div class="form-group form-inline p-2 bg-primary text-white">
 				...
 				<select class="form-control input-sm" id="select_category">
 					<option value="0" <?php if ($this->uri->segment(3) == 0) {
																echo 'selected';
															} ?>>Belum Ditindak Lanjut</option>
 					<option value="1" <?php if ($this->uri->segment(3) == 1) {
																echo 'selected';
															} ?>>Sudah Ditindak Lanjut</option>
 				</select>
 			</div>
 		</div>
 					...
 					<tbody>
 						<?php
							...
							foreach ($data as $row) {
								...
								// $status_rekam = "Sudah Direkam";

								// if ($row->status_rekam == 0) { 
								//     $status_rekam = "Belum Direkam";
								// }   
							?>
 							<tr <?= $var_bg ?>>
 								...
 								<!-- <td align="center"><(($row->sp2dk_nomor != null) || ($row->sp2dk_nomor != "")) ? "Sudah Direkam":"Belum Direkam"?></td> -->
 								<!--  <td align="center"><$status_rekam?></td> -->
 								<td align="center">
 									<?php if ($row->tindak_lanjut == 0) {
											if ($this->session->userdata('level') == 1 ||  $this->session->userdata('level') == 5 || $this->session->userdata('level') >= 7) {
												echo anchor(site_url('data/tindak_lanjut_update/' . $row->id), 'Tindak Lanjut', array('class' => 'btn btn-sm btn-success btn-flat'));
												echo ' ';
											} ?>
 										...
 										<!-- echo anchor(site_url('data/tindak_lanjut_read/' . $row->id), '<i class="fa fa-eye "></i>', array('class' => 'btn btn-sm btn-primary btn-flat')); -->
 									<?php } else {
											if ($this->session->userdata('level') == 1 ||  $this->session->userdata('level') == 5 || $this->session->userdata('level') >= 7) {
												echo anchor(site_url('data/tindak_lanjut_update/' . $row->id), 'Ubah', array('class' => 'btn btn-sm btn-success btn-flat'));
												echo ' ';
											} ?>
 										<button type="button" id="detail" class="btn btn-sm btn-primary detail" data-id="<?= $row->id ?> " data-toggle="modal" data-target="#detailModal" data-tooltip="tooltip" data-placement="bottom" title="Detail"><i class="fa fa-eye"></i></button>
 										<!-- echo anchor(site_url('data/tindak_lanjut_read/' . $row->id), '<i class="fa fa-eye "></i>', array('class' => 'btn btn-sm btn-primary btn-flat')); -->
 									<?php }
										...
										// echo anchor(site_url('data/tindak_lanjut_rekam/' . $row->id),
										//     'Rekam', array('class' => 'btn btn-sm btn-danger btn-flat')) -->
										?>
 								</td>
 							</tr>
 						<?php
 ...
 <div class="modal fade" id="eksporModal" tabindex="-1" role="dialog" aria-labelledby="eksporModalLabel" aria-hidden="true">
 	<div class="modal-dialog" role="document">
 		<div class="modal-content">
 			...
 			<div class="modal-body">
 				<form method="POST" action="<?= base_url() ?>cetak/tindak_lanjut" target="_blank">
 ...
 <div class="modal fade bd-example-modal-lg" tabindex="-1" id="detailModal">
 	<div class="modal-dialog modal-lg">
 		<div class="modal-content">
 			<div class="modal-header">
 				<h5 class="modal-title"><b>Detail Tindak Lanjut SP2DK-LHP2DK</b></h5>
 				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
 					<span aria-hidden="true">&times;</span>
 				</button>
 			</div>
 ...
 <script>
 	$(document).ready(function() {
 		$('#select_category').change(function() {
 			var val = $(this).val();
 			if (val == 0) {
 				window.location = '<?php echo site_url('data/tindak_lanjut') ?>';
 			} else {
 				window.location = '<?php echo site_url('data/tindak_lanjut/') ?>' + val;
 			}
 		});
 	});
 </script>
