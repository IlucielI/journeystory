								...
								<td align="center">
									<?php if ($this->session->userdata('level') == 1 || $this->session->userdata('level') == 3 || $this->session->userdata('level') == 4) : ?>
										<?php if ($row->verifikasi == 0) {
											echo anchor(site_url('data/verifikasi_material_verif/' . $row->id), '<button type="button" class="btn btn-sm btn-success btn-flat" data-tooltip="tooltip" data-placement="bottom" title="Verifikasi"><i class="fas fa-clipboard-check"></i></button>', array('class' => ''));
											echo ' '; ?>
											<!-- // echo anchor(site_url('data/verifikasi_material_batal/' . $row->id), 'Dibatalkan', array('class' => 'btn btn-sm btn-warning btn-flat'));
											// echo ' ';
											// echo anchor(site_url('data/verifikasi_material_read/' . $row->id), '<i class="fa fa-eye "></i>', array('class' => 'btn btn-sm btn-primary btn-flat'));
											// echo ' ';
											// echo anchor(site_url('data/verifikasi_material_delete/' . $row->id), '<i class="fa fa-trash-o "></i>', array('class' => 'btn btn-sm btn-danger btn-flat')); -->
									<?php }
									endif; ?>
...
<div class="modal fade" id="eksporModal" tabindex="-1" role="dialog" aria-labelledby="eksporModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST" action="<?= base_url() ?>cetak/verifikasi_material" target="_blank">
...
<div class="modal fade bd-example-modal-lg" tabindex="-1" id="detailModal">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title"><b>Detail Verifikasi Material</b></h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script>
	$(document).ready(function() {
		$('#select_category').change(function() {
			var val = $(this).val();
			if (val == 0) {
				window.location = '<?php echo site_url('data/verifikasi_material') ?>';
			} else {
				window.location = '<?php echo site_url('data/verifikasi_material/') ?>' + val;
			}
		});
	});
</script>
